            ...
            this.serverPromise = this[SYMBOL_LOAD_CONFIG]().then(async (conf)=>{
                if (this.options.dev) {
                    if (conf.experimental.lockDistDir) {
                        const dir = _nodepath.default.resolve(/* turbopackIgnore: true */ this.options.dir || '.');
                        const distDir = _nodepath.default.join(/* turbopackIgnore: true */ dir, conf.distDir);
                        (0, _nodefs.mkdirSync)(distDir, {
                            recursive: true
                        });
                        this.lockfile = await _lockfile.Lockfile.acquireWithRetriesOrExit(_nodepath.default.join(/* turbopackIgnore: true */ distDir, 'lock'), 'next dev');
                    }
                } else {
